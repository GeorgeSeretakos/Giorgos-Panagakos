...
export default function BookPage() {
  // Locale – default EN, pick up saved choice if any
  ...
  useEffect(() => {
    async function run() {
      ...
      setError(null);
      try {
        const radiusFromEnv = Number.parseFloat(
          process.env.NEXT_PUBLIC_SEARCH_RADIUS_KM ?? ""
        );
        ...
        setError(
          locale === "en"
            ? "Failed to search for nearby studios."
            : "Αποτυχία αναζήτησης κοντινών studios."
        );
      } finally {
        setLoading(false);
      }
  ...
  return hasInput ? (
    ...
    <BookLandingLayout locale={locale} steps={steps} onSearch={handleSearch} />
  );
}
